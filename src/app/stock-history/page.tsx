    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-green-800 text-white px-6 py-3 shadow flex items-center gap-3">
        ...
        <div>
          ...
          {productName && (
            <p className="text-green-200 text-xs mt-0.5">{decodeURIComponent(productName)}</p>
          )}
        </div>
      </header>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={row.id} className="bg-white even:bg-gray-50 border-t border-gray-100">
                    ...
                    <td className={`px-4 py-2 text-right font-medium whitespace-nowrap ${isAdd ? 'text-green-700' : 'text-orange-600'}`}>
