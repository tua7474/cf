...
export default function StockHistoryPage({
  ...
  searchParams: Promise<{ id?: string; type?: string; name?: string }>
}) {
  ...
  useEffect(() => {
    if (!stockId) return
    setLoading(true)
    fetch(`/api/stock/history?id=${stockId}&type=${type}`)
      .then(r => r.json())
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-gray-500 text-white px-6 py-3 shadow flex items-center gap-3">
        ...
        <div>
          ...
          {productName && (
            <p className="text-orange-200 text-xs mt-0.5">{decodeURIComponent(productName)}</p>
          )}
        </div>
      </header>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={row.id} className="bg-white even:bg-gray-50 border-t border-gray-100">
                    ...
                    <td className={`px-4 py-2 text-right font-medium whitespace-nowrap ${isAdd ? 'text-orange-300' : 'text-orange-300'}`}>
                      {isAdd ? '+' : '−'}{fmtQty(row.qty)}
                    </td>
                  </tr>
                ))}
              </tbody>
