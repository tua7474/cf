    ...
    return (
      <div style={{ width: '210mm', height: '297mm', padding: '8mm', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', fontFamily: 'sans-serif' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '3mm' }}>
          <div style={{ fontSize: '16pt', fontWeight: 'bold', color: '#fb923c' }}>ใบจองสินค้า</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                ...
                {Array.from(modelMap.entries()).flatMap(([modelName, items]) =>
                  items.map((item, idx) => (
                    <tr key={`${modelName}-${idx}`} style={{ backgroundColor: idx % 2 === 0 ? 'white' : '#f5f5f5' }}>
                      {idx === 0 && (
                        <td rowSpan={items.length} style={{ ...tdBase, fontWeight: 'bold', color: '#fb923c', textAlign: 'center', verticalAlign: 'middle', backgroundColor: '#ccfbf1', fontSize: '7.5pt' }}>
                          {modelName}
                        </td>
                      )}
              ...
              <tfoot>
                <tr style={{ backgroundColor: '#ccfbf1' }}>
                  <td colSpan={4} style={{ ...tdBase, textAlign: 'right', fontWeight: 'bold', fontSize: '10pt', borderColor: '#888' }}>ยอดเงินรวม</td>
                  <td style={{ ...tdBase, textAlign: 'right', fontWeight: 'bold', fontSize: '10pt', color: '#fb923c', borderColor: '#888' }}>
                  ...
                  {(isAdmin ? orders : orders.filter(o => o.branch_name === branchName)).map((order, i) => {
                    ...
                    return (
                      <tr key={order.id} className={cancelled ? 'bg-red-50 opacity-60' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        {/* 1. เลขที่ใบจอง */}
                        <td className="px-4 py-3 border-r border-gray-200 font-mono font-bold text-orange-400 text-base">
                          {order.order_no}
                        </td>
                        ...
                        <td className="px-4 py-3 border-r border-gray-200">
                          <div className="flex flex-col gap-1.5">
                            <div className="flex justify-between items-center gap-2 text-[11px]">
                              <span className={order.vehicle_type ? (order.vehicle_type === 'จองรถ60000' ? 'text-blue-700 font-medium' : 'text-orange-400 font-medium') : 'text-gray-300'}>
                        ...
                        <td className="px-4 py-3 border-r border-gray-200 text-center">
                          {cancelled ? (
                            ...
                            <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-orange-400">
                              ✅ ขึ้นของแล้ว
                            </span>
                          ) : isAdmin ? (
                            <button
                        ...
                        <td className="px-4 py-3 border-r border-gray-200">
                          {cancelled ? (
                            ...
                            <div className="flex flex-col gap-0.5">
                              <span className="inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-orange-400">
                                ✅ ชำระแล้ว
                              </span>
                              {order.payment_date && (
